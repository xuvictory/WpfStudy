...
public sealed class CartServiceTests
{
    ...
    {
        _settings = new SettingsService(_eventAggregator);
        _cart = new CartService(_repository, _settings, _eventAggregator);
    }
    ...
    public void SummaryAndPublish_TrackCurrentState()
    {
        ...
        // keepSubscriberReferenceAlive: true —— 用 lambda 订阅时必须显式保活，
        // 否则 Prism 的弱引用订阅可能在 GC 后被回收，导致断言随机失败。
        _eventAggregator.GetEvent<CartChangedEvent>()
            .Subscribe(summary => captured = summary, keepSubscriberReferenceAlive: true);
